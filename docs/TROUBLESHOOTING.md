...
**Problem**: `/spec-feature` or other OpenCode commands not recognized.

**Solution**:

1. Ensure you're using OpenCode
2. Commands are in `.opencode/commands/` directory
3. Restart the OpenCode session if needed
4. Check command file has `.md` extension

### Changeset Guidance Issues

**Problem**: Unsure when to add changesets with OpenCode AI assistance.

**Solution**:

- **Add changeset** for: bug fixes, features, breaking changes
- **Use --empty** for: tests, CI changes, documentation, refactoring
- OpenCode commands should remind you about changesets
- Check `pnpm changeset:status` to see pending changes

### AI Getting Import Paths Wrong

**Problem**: OpenCode suggests imports without `.js` extensions.

**Solution**:

- Remind OpenCode about the ES modules requirement
